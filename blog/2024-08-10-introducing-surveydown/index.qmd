...
[supabase](https://supabase.com/). The video on [this page]() offers a 5 minute overview of the platform.
...
After getting everything [installed](https://jhelvy.github.io/surveydown/articles/installation.html), we recommend starting with a template survey project. To do so, run the following in the R console:
...
Once you are happy with your survey, you can deploy it live to any server of your choice. Since it's a shiny app, you can [deploy it to shinyapps.io](https://quarto.org/docs/interactive/shiny/running.html#shinyapps) for free! 
...
The only other open-source survey platform I have used is [formr](https://formr.org/), which is a very powerful platform, but it is rather clunky to use (you define your survey in Google sheet cells), and it is not as easy to edit as a simple markdown file. 

What we've come up with is a survey platform that is flexible, easy to use, and built entirely with open-source technologies. I think it solves a lot of problems, and hopefully you'll agree.
...
- **Start at a specific page when editing**: in the `sd_config()` function, you can define a `start_page` argument that will skip to a specific page when the survey is first loaded.
- **Show all pages at once when editing**: in the `sd_config()` function, you can set `show_all_pages = TRUE` to show all the pages at once when the survey is first loaded.
- **Define custom complex skip logic**: in the `sd_config()` function, you can define a `skip_if_custom` argument that allows you to define any custom function that if true will skip to a specific page.
- **Define custom conditional display logic**: in the `sd_config()` function, you can define a `show_if_custom` argument that allows you to define any custom function that if true will show a specific question.

## Did I mention you can run R code in your survey?

Every surveydown survey uses R code chunks for questions. But you can also insert R code for all sorts of other things. For example, if you wanted to randomize the order of the questions in your survey, you could write some R code for that. Want to insert a plot of something? Write a little ggplot code. You can also add interactive components to your surveys, such as showing a respondent how their responses compare to others in real time.
...
- Want others to see your survey live? Just turn off the database by setting `db <- sd_database()` with no arguments, and your app will function without a database.
