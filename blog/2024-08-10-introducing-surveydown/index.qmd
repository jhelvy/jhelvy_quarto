...
The [{surveydown} package](https://jhelvy.github.io/surveydown/) provides a set of functions for defining the survey content and control logic. Each function starts with `sd_` to make them easy to identify.
...
Every surveydown survey uses R code chunks for questions. But you can also insert R code for all sorts of other things. For example, if you wanted to randomize the order of the questions in your survey, you could write some R code for that. Want to insert a plot of something? Write a little ggplot code. We're also working on adding the ability to work with the data from the survey itself so that you could do some interactive things, like show a respondent how their responses compare to others in real time.
