...
Some courses have restrictions on who can register for a variety of reasons. Often times the restriction is due to missing prerequisite courses or the course is only offered to majors in that field (e.g. most courses in DATS are restricted to DATS majors). In these cases, you typically need to fill out an [RTF]({{< var rtf_url >}}){target="_blank"} form. Here are some details:
...
- For CSCI courses: Go to the [CS Department’s Registration help form](https://go.gwu.edu/csregs) and fill out the request form. Dr. Tim Wood will take care of processing the request.
